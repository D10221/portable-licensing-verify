import crypto from "crypto";
import * as sampleData from "./sample-data";

describe("RSA sign, verify", () => {

  it("License Manager Works", () => {
    /**
 * License manageerr Keys
 * public: is `Asn1 Pkcs` , der encoded
 * salt = byte[16]; // random

 * )
 */
    const Keys = {
      /** 
       * base64 encoded, encrypted, passPhrase = "test" 
       * ToBase64String (
       *    privateKeyFactory.EncryptKey(
       *        keyEncryptionAlgorithm // PbeWithShaAnd3KeyTripleDesCbc.id : DerObjectIdentifier : Asn1Object,
       *        passPhrase.ToCharArray(),
       *        salt, iterationCount: 10, key)
       * */
      pvtKey: `""`,
      /** Asn1 Pkcs encoded */
      pubKey: `"`,
    };
    const { pvtKey, pubKey } = Keys;
    var data = "xyz";
    let signature;
    {
      const sign = crypto.createSign("RSA-SHA256");
      sign.update(data);
      const k = crypto.createPrivateKey(pemEncode(pvtKey, "private"));
      signature = sign.sign(k);
    }
    {
      const verify = crypto.createVerify("RSA-SHA256");
      verify.update(data);
      expect(verify.verify(pemEncode(pubKey, "public"), signature)).toBe(true);
    }
  });

  it("Standard works", () => {
    /**
     * Standard Keys
     */
    const Keys = {
      pubKey:
        "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCbbBSVpWzSmCGVeezhuVFgUEYowUxgX/SnFdymGRCHGc77d5I0xkMAnIOWbI2MmP8j/7sdfPuUF0V5zw+Hd/7iZ6vs2k4JRKdprrB/zSC4GGqCDpDkbRYydcw3kwDgKkHhDp6NwIKvvl87WsnFozi487tGPQ8NO15hngwsV7DrawIDAQAB",
      pvtKey:
        "",
    };
    const { pvtKey, pubKey } = Keys;
    var data = "xyz";
    let signature;
    {
      const sign = crypto.createSign("RSA-SHA256");
      sign.update(data);
      signature = sign.sign(pemEncode(pvtKey, "private"));
    }
    {
      const verify = crypto.createVerify("RSA-SHA256");
      verify.update(data);
      expect(verify.verify(pemEncode(pubKey, "public"), signature)).toBe(true);
    }
  });
});

const EOL = "\n";
function pemEncode(key: string, mode: "private" | "public" | "cert") {
  switch (mode) {
    case "public":
      return (
        "-----BEGIN PUBLIC KEY-----" +
        EOL +
        key +
        EOL +
        "-----END PUBLIC KEY-----"
      );
    case "private":
      return (
        "-----BEGIN PRIVATE KEY-----" +
        EOL +
        key +
        EOL +
        "-----END PRIVATE KEY-----"
      );
    case "cert":
      return (
        "-----BEGIN CERTIFICATE-----" +
        EOL +
        key +
        EOL +
        "-----END CERTIFICATE-----"
      );
    default:
      throw new Error("Invalid mode: " + mode + " expected 'private'|'public'");
  }
}

